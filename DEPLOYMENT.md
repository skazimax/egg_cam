...
Монитор считает только рост максимального числа яиц в текущей сессии. Новое
яйцо подтверждается тремя дополнительными кадрами с интервалом 5 секунд. Для
сброса нужны: отсутствие яиц, отсутствие курицы в `nest_zones` и сходство с
эталоном на трёх дополнительных кадрах, после чего выполняется финальная
...
### Новые яйца видны, но не создаются события

Проверьте сохранённый максимум текущей сессии и последние решения детектора:

```bash
sqlite3 runtime/production/events.sqlite3 \
  "SELECT key, value FROM metadata WHERE key = 'inventory_session_peak';"

journalctl -u egg-cam.service --since today --no-pager | \
  grep 'frame=.*visible='
```

Если `session_peak` остался от предыдущей кладки, автоматическое подтверждение
пустых гнёзд могло не сработать из-за курицы, изменившейся соломы или
освещения. Не удаляйте события для исправления состояния. Перед ручным сбросом
сделайте резервную копию SQLite и сохраните диагностические кадры. План
устранения этой зависимости приведён в
